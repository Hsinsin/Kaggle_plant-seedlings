...
class Train_Engine(object):
        ...
        with open('acc.txt','w') as f:
            for epoch in range(0, epochs):
                ...
                if test_data is not None and eval_step and (epoch + 1) % eval_step == 0:
                    loss_valid, acc_valid = self.eval_func(test_data, criterion)
                    losses_valid.append(loss_valid)
                    accs_valid.append(acc_valid)
                    """
                    is_best = accs_valid > best_test_acc
                    if is_best:
                        best_test_acc = accs_valid
            ...
            plt.subplot(121)
            plt.plot(nums, losses_train, color='blue', label='train_loss')
            plt.plot(nums, losses_valid, color='red', label='valid_loss')
            plt.xlabel("num")
            plt.ylabel("loss") 
            plt.title('loss')
            plt.subplot(122)
            plt.plot(nums, accs_train, color='blue', label='train_accuracy')
            plt.plot(nums, accs_valid, color='red', label='valid_accuracy')
            plt.xlabel("num")
            plt.ylabel("accuracy") 
            plt.title('accuracy')
    ...
    """
    def eval_func(self, test_data, criterion):
        self.net.eval()
        total_loss = 0.0
        correct = 0
        total = 0
